...
CREATE VIEW planned_hours AS 
 WITH calculated_hours AS (
         SELECT cl.course_code AS "Course Code",
          ...
          WHERE EXTRACT(year FROM ci.start_date) = 2025
...
/* SEM2 view2 */
CREATE OR REPLACE VIEW allocated_hours_final AS
WITH allocated_per_teacher AS (
    SELECT
        ...
        SUM(
            ...
            SELECT ta_1.factor
            FROM teaching_activity ta_1
            WHERE ta_1.activity_name::text = 'Lecture'::text
        ) AS "Lecture Hours",
        SUM(
            CASE
            ...
            SELECT ta_1.factor
            FROM teaching_activity ta_1
            WHERE ta_1.activity_name::text = 'Tutorial'::text
        ) AS "Tutorial Hours",
        SUM(
            CASE
            ...
            SELECT ta_1.factor
            FROM teaching_activity ta_1
            WHERE ta_1.activity_name::text = 'Lab'::text
        ) AS "Lab Hours",
        SUM(
            CASE
            ...
            SELECT ta_1.factor
            FROM teaching_activity ta_1
            WHERE ta_1.activity_name::text = 'Seminar'::text
        ) AS "Seminar Hours",
        SUM(
            CASE
            ...
            SELECT ta_1.factor
            FROM teaching_activity ta_1
            WHERE ta_1.activity_name::text = 'Administration'::text
        ) AS "Admin",
        SUM(
            CASE
            ...
            SELECT ta_1.factor
            FROM teaching_activity ta_1
            WHERE ta_1.activity_name::text = 'Examination'::text
        ) AS "Exam"
    FROM course_instance ci
    JOIN (
        SELECT DISTINCT
            ...
            course_layout.course_layout_id
        FROM course_layout
    ) cl ON ci.course_layout_id = cl.course_layout_id
    JOIN course_credits cc
        ON cc.course_layout_id = ci.course_layout_id
       AND cc.study_period = ci.study_period
    JOIN planned_activity pa
        ON ci.instance_id = pa.instance_id
    JOIN activity_allocation aa
        ON aa.instance_id = ci.instance_id
 
    JOIN employee emp
        ON aa.employment_id = emp.employment_id
    JOIN person per
...
SELECT
    ...
    ("Lecture Hours"
     ...
     + "Seminar Hours"
     + "Admin"
     + "Exam") AS "Total Calculated Hours"
FROM allocated_per_teacher
ORDER BY "Course Code", "Teacher's Name";
